...
$page_title = 'Επεξεργασία Στοιχείων Μαθητή';
include('includes/header.php');
print("<br>");
print("<h1>Επεξεργασία Στοιχείων Μαθητή</h1>");
    ...
    if (filter_input(INPUT_POST, 'submit')) {
        ...
        if ($email = filter_input(INPUT_POST, "email", FILTER_SANITIZE_STRING)) {
            if (!$email = filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                my_mysqli_stmt_bind_param($stmt, 'si', $email, $studentid);
        ...
        if (empty($errors)) {
            ...
            $stmt = my_mysqli_prepare($dbc, $q);
            my_mysqli_stmt_bind_param($stmt, 'ssssi', $firstname, $lastname, $email, $phone, $studentid);
            my_mysqli_stmt_execute($stmt);
